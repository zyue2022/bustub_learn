...
HashJoinExecutor::HashJoinExecutor(ExecutorContext *exec_ctx, const HashJoinPlanNode *plan,
    ...
    : AbstractExecutor(exec_ctx),
      plan_(plan),
      left_child_executor_(move(left_child)),
      right_child_executor_(move(right_child)) {}
    ...
    if (map_.count(dis_key) != 0) {
      for (auto &left_tuple_new : map_.find(dis_key)->second) {
        ...
        for (const auto &col : GetOutputSchema()->GetColumns()) {
          output.emplace_back(col.GetExpr()->EvaluateJoin(&left_tuple_new, left_child_executor_->GetOutputSchema(),
